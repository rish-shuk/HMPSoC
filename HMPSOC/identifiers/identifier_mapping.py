...
def write_mif_file(config_buttons, binary_identifiers, output_file):
    with open(output_file, 'w') as file:
        ...
        for i in range(len(config_buttons)):
            ...
            file.write("{} : {};\n".format(3*i + 1, config_button))
            file.write("{} : {};\n".format(3*i + 2, first_16_bits))
            file.write("{} : {};\n".format(3*i + 3, last_16_bits))
        file.write("END;\n")

def main():
    config_buttons_file = input("Enter the path to the config buttons file: ")
    binary_identifiers_file = input("Enter the path to the binary identifiers file: ")
    output_file = input("Enter the path to the output .mif file: ")
